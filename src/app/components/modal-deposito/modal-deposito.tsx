...
export function ModalDeposito({ isOpen, onClose, deposito }: Props) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-4">
      <div className="fixed h-screen w-full bg-black bg-opacity-50" onClick={onClose}></div>
      <div className="relative bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
        ...
        <div>
          ...
          <form className="grid gap-2 mt-4" onSubmit={handleUpdateTransaction}>
            <select name="tipo" id="tipo" value={typeTransaction} onChange={(e) => setTypeTransaction(e.target.value)} className="selectNewTransaction w-full h-12 pl-4 border-[1px] focus:border-my-green border-my-blue rounded-lg text-black appearance-none outline-none">
