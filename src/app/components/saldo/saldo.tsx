...
export default function Saldo() {
    ...
    return (
        <div className="bg-my-blue w-full p-6 md:flex justify-between pb-36 rounded-lg text-white">
            <div>
                <h2 className="text-2xl mb-4">Olá, <Link href="/profile">Joana!</Link> :)</h2>
                <p>{formatarData()}</p>
            </div>
            <div className="mt-12 md:mr-6 lg:mr-20">
                <p className="font-bold text-xl flex items-center relative align-middle gap-6 cursor-pointer w-fit" onClick={toggleValueVisibility}>
                    Saldo
                    <span className={`relative before:content-[''] before:w-1 before:h-7 ${!showValue ? 'before:block' : 'before:hidden'}  before:absolute before:top-1/2 before:left-1/2 before:bg-white before:-rotate-45 before:-translate-x-1/2 before:-translate-y-1/2`}>
                        <svg className={`relative `} width="20" height="14" viewBox="0 0 20 14" fill="none" xmlns="http://www.w3.org/2000/svg">
